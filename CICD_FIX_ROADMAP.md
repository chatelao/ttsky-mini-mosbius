...
- [ ] **4.1 Continuous CI/CD Configuration Drift & Rule Set Maintenance**
  - [ ] **4.1.1 Track Upstream Actions & PDK Toolchain Updates**
    ...
    - [ ] **4.1.1.3 Monitor TinyTapeout upstream release channels for future PDK tag upgrades beyond `@ttihp26b`**
      - [ ] **4.1.1.3.1** Periodically query upstream `TinyTapeout/tt-gds-action` release tags for new IHP SG13G2 tag releases.
      - [ ] **4.1.1.3.2** Review release changelogs for breaking changes in container image layers, precheck DEF templates, or Pages API parameters.
      - [ ] **4.1.1.3.3** Create staging branch to test new action tags in `.github/workflows/gds.yaml` and `.github/workflows/docs.yaml`.
  - [ ] **4.1.2 Maintain Static Verification Assertions & Workflow Schema Integrity**
    ...
    - [ ] **4.1.2.3 Update assertion functions in `py/verify_cicd_config.py` whenever workflow triggers, steps, or permissions are modified**
      - [ ] **4.1.2.3.1** Audit `check_gds_workflow` and `check_docs_workflow` when adding new CI steps or environment variables.
      - [ ] **4.1.2.3.2** Audit `check_pages_deployment_and_oidc_config` when modifying job authorization or token scopes.
      - [ ] **4.1.2.3.3** Add corresponding mock unit tests in `py/test_verify_cicd_config.py` for new static verification assertions.
...
- [ ] **4.2 Precheck Artifact & Toolchain Verification Sign-Off**
  - [ ] **4.2.1 Precheck Artifact & Template Validation**
    ...
    - [ ] **4.2.1.3 Inspect generated precheck reports and log artifacts following future toolchain minor version upgrades**
      - [ ] **4.2.1.3.1** Download and extract precheck build summary zip files from GitHub Actions run artifacts.
      - [ ] **4.2.1.3.2** Verify boundary, layer, and pin placement check log outputs in `precheck.log`.
      - [ ] **4.2.1.3.3** Confirm zero warnings or non-blocking notices are introduced by toolchain updates.
  - [ ] **4.2.2 DRC, Antenna & Geometry Verification Rule Check Compatibility**
    ...
    - [ ] **4.2.2.3 Re-verify KLayout DRC execution clean status when updating SG13G2 PDK technology files**
      - [ ] **4.2.2.3.1** Execute local Magic DRC script (`tcl/magic_drc.tcl`) against updated PDK technology files.
      - [ ] **4.2.2.3.2** Run KLayout DRC rule file (`sg13g2.lydrc`) in batch mode against `gds/tt_um_tnt_mosbius.gds`.
      - [ ] **4.2.2.3.3** Confirm antenna, density, and zero-area geometry reports contain zero rule violations.
  - [ ] **4.2.3 Documentation Rendering & Schema Compatibility**
    ...
    - [ ] **4.2.3.3 Maintain documentation rendering compatibility when adding new schema parameters to `info.yaml`**
      - [ ] **4.2.3.3.1** Verify `info.yaml` schema updates pass static validation in `py/verify_cicd_config.py`.
      - [ ] **4.2.3.3.2** Verify Markdown and HTML rendering in `docs` job generates pinout diagrams without formatting errors.
