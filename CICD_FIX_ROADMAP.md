...
- [ ] **4.1 Continuous CI/CD Configuration Drift & Rule Set Maintenance**
  - [ ] **4.1.1 Track Upstream Actions & PDK Toolchain Updates**
    ...
    - [ ] **4.1.1.3** Monitor TinyTapeout upstream release channels for future PDK tag upgrades beyond `@ttihp26b`.
      - [ ] **4.1.1.3.1** Monitor `TinyTapeout/tt-gds-action` release notes and repository tags for new `ihp-sg13g2` releases.
      - [ ] **4.1.1.3.2** Test candidate tag upgrades in an isolated development branch before updating production workflows.
      - [ ] **4.1.1.3.3** Update expected tag constants in `py/verify_cicd_config.py` concurrently with workflow file tag updates.
  - [ ] **4.1.2 Maintain Static Verification Assertions & Workflow Schema Integrity**
    ...
    - [ ] **4.1.2.3** Update assertion functions in `py/verify_cicd_config.py` whenever workflow triggers, steps, or permissions are modified.
      - [ ] **4.1.2.3.1** Audit `py/verify_cicd_config.py` rule functions during workflow structure modifications.
      - [ ] **4.1.2.3.2** Synchronize matching positive and negative mock test cases in `py/test_verify_cicd_config.py`.
      - [ ] **4.1.2.3.3** Execute `make check` locally to ensure no verification regressions occur.
...
- [ ] **4.2 Precheck Artifact & Toolchain Verification Sign-Off**
  - [ ] **4.2.1 Precheck Artifact & Template Validation**
    ...
    - [ ] **4.2.1.3** Inspect generated precheck reports and log artifacts following future toolchain minor version upgrades.
      - [ ] **4.2.1.3.1** Review `precheck.log` and `precheck_results.xml` artifacts after CI precheck job runs.
      - [ ] **4.2.1.3.2** Verify no new non-fatal warnings or rule suppressions are introduced by upstream container updates.
      - [ ] **4.2.1.3.3** Archive sign-off logs alongside precheck summary reports in release build documentation.
  - [ ] **4.2.2 DRC, Antenna & Geometry Verification Rule Check Compatibility**
    ...
    - [ ] **4.2.2.3** Re-verify KLayout DRC execution clean status when updating SG13G2 PDK technology files.
      - [ ] **4.2.2.3.1** Run local precheck script `py/run_precheck.py` against updated PDK geometry definitions.
      - [ ] **4.2.2.3.2** Validate KLayout DRC output for zero violations across antenna, density, and min-width rules.
      - [ ] **4.2.2.3.3** Confirm prBoundary layer 189/4 boundary alignment across modified layout macros.
  - [ ] **4.2.3 Documentation Rendering & Schema Compatibility**
    ...
    - [ ] **4.2.3.3** Maintain documentation rendering compatibility when adding new schema parameters to `info.yaml`.
      - [ ] **4.2.3.3.1** Verify `info.yaml` additions against TinyTapeout schema specification.
      - [ ] **4.2.3.3.2** Validate static docs rendering rules in `check_docs_build_and_asset_config` for new parameters.
      - [ ] **4.2.3.3.3** Verify generated HTML documentation preview in `docs.yaml` workflow artifacts.
