...
| **5** | **Precheck Tile Template Resolution** | GitHub Actions (`precheck@ttihp26b`) | `tech/ihp-sg13g2/def/analog/tt_analog_3x2_3v3.def` | Submodule DEF template file establishing 3x2 analog tile boundaries and pin placement rules | ❌ **Fail** | **Pin Check / Boundary Check / Analog Pin Check:** `[Errno 2] No such file or directory: '../tech/ihp-sg13g2/def/analog/tt_analog_3x2_3v3.def'`. Relative path resolution failed in `tt/precheck` subdirectory due to missing tech symlinks (`tt/tech` and `tt/precheck/tech`). |
