...
"""
    ...
    def test_check_precheck_report_artifacts_missing_def(self):
    ...
    def test_check_pdk_drc_rule_compatibility_valid(self):
        valid_drc = "drc check\n"
        valid_fix_gds = "189 -> 235 remapping logic"

        def mock_open_file(filepath, mode="r"):
            if "magic_drc.tcl" in filepath:
                return unittest.mock.mock_open(read_data=valid_drc)()
            else:
                return unittest.mock.mock_open(read_data=valid_fix_gds)()
    ...
    def test_check_pdk_drc_rule_compatibility_invalid(self):
        invalid_drc = "# No DRC check"
        invalid_fix_gds = "# Missing layer maps"

        def mock_open_file(filepath, mode="r"):
            if "magic_drc.tcl" in filepath:
                return unittest.mock.mock_open(read_data=invalid_drc)()
            else:
                return unittest.mock.mock_open(read_data=invalid_fix_gds)()
