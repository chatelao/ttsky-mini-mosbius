    ...
    for path in [gds_path, docs_path]:
        ...
        if not os.path.exists(path):
            ...
            if "name:" not in content:
                errors.append(f"Missing 'name:' top-level declaration in {filename}")
...
def check_pdk_drc_rule_compatibility(repo_root="."):
    ...
    if not os.path.exists(gds_workflow_path):
        ...
        with open(gds_workflow_path, "r") as f:
            gds_content = f.read()
        if "TinyTapeout/tt-gds-action/precheck@ttihp26b" not in gds_content:
            errors.append("Missing 'precheck@ttihp26b' action in gds.yaml for PDK DRC rules")
    ...
    if not os.path.exists(info_path):
        ...
        for key in required_keys:
            if key not in content:
                errors.append(f"Missing required schema field '{key}' in info.yaml")
