...
def check_upstream_pdk_action_tags(repo_root="."):
    ...
    action_pattern = re.compile(r"TinyTapeout/tt-gds-action/([a-zA-Z0-9_-]+)@([a-zA-Z0-9_-]+)")
    ...
    for path in [gds_path, docs_path]:
        ...
        if not os.path.exists(path):
            ...
            if not matches:
                errors.append(f"No TinyTapeout actions found in {filename}")
            for action_name, tag in matches:
                if tag != expected_tag:
                    errors.append(f"Action 'TinyTapeout/tt-gds-action/{action_name}' in {filename} uses tag '{tag}' instead of expected '{expected_tag}'")
    ...
    for path in [gds_path, docs_path]:
        ...
        if not os.path.exists(path):
            ...
            if "name:" not in content or "on:" not in content or "jobs:" not in content:
                errors.append(f"Workflow schema structure incomplete in {filename}")
...
def check_precheck_report_artifacts(repo_root="."):
    ...
    if not os.path.exists(gds_workflow_path):
        ...
        with open(gds_workflow_path, "r") as f:
            content = f.read()
        if "precheck:" not in content or "TinyTapeout/tt-gds-action/precheck@ttihp26b" not in content:
            errors.append("Missing precheck step producing precheck report artifacts in gds.yaml")
...
def check_info_yaml_schema_compatibility(repo_root="."):
    ...
    if not os.path.exists(info_path):
        ...
        with open(info_path, "r") as f:
            content = f.read()
        required_fields = ["project:", "top_module:", "language:", "tiles:", "uses_vapwr:", "analog_pins:", "pinout:", "yaml_version:"]
        for field in required_fields:
            if field not in content:
                errors.append(f"Schema compatibility issue: missing required field '{field}' in info.yaml")
