        ...
        if not os.path.exists(path):
            ...
            for tag in tags:
                if tag not in content:
                    errors.append(f"Missing expected action tag '{tag}' in {filename}")
            if "ttsky26c" in content:
    ...
    for path in [gds_path, docs_path]:
        ...
        if not os.path.exists(path):
            ...
            if not re.search(r"^name:\s*\S+", content, re.MULTILINE):
                errors.append(f"Missing top-level 'name' attribute in {filename}")
            if not re.search(r"^on:\s*", content, re.MULTILINE):
                errors.append(f"Missing top-level 'on' trigger block in {filename}")
            if not re.search(r"^jobs:\s*", content, re.MULTILINE):
                errors.append(f"Missing top-level 'jobs' block in {filename}")
            if "runs-on: ubuntu-24.04" not in content:
                errors.append(f"Missing 'runs-on: ubuntu-24.04' runner attribute in {filename}")
...
def check_info_yaml_schema_compatibility(repo_root="."):
    ...
    if not os.path.exists(info_path):
        ...
        required_keys = [
            r"top_module:\s*[\"']?tt_um_tnt_mosbius[\"']?",
            r"language:\s*[\"']?Analog[\"']?",
            r"uses_vapwr:\s*true",
            r"tiles:\s*[\"']?3x2[\"']?",
            r"analog_pins:\s*6",
        ...
        for req in required_keys:
            if not re.search(req, content, re.IGNORECASE):
                key_label = req.split(":")[0].replace("\\", "")
                errors.append(f"Missing or invalid key/value pattern '{key_label}' in info.yaml")
