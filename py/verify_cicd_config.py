...
def check_upstream_pdk_action_tags(repo_root="."):
    ...
    for path in [gds_path, docs_path]:
        ...
        if not os.path.exists(path):
            ...
            for tag in tags:
                if tag != "ttihp26b":
                    errors.append(f"Invalid or untracked action tag '@{tag}' in {filename} (expected '@ttihp26b')")
...
def check_info_yaml_schema_compatibility(repo_root="."):
    ...
    for key in required_keys:
        if key not in content:
            errors.append(f"Missing required key '{key}' in info.yaml")
...
def check_precheck_report_artifacts(repo_root="."):
    def_template_path = os.path.join(
        repo_root, "tech/ihp-sg13g2/def/analog/tt_analog_3x2_3v3.def"
    )
    gds_workflow_path = os.path.join(repo_root, ".github/workflows/gds.yaml")
    errors = []

    if not os.path.exists(def_template_path):
        errors.append(f"Missing required precheck DEF template file: {def_template_path}")

    if not os.path.exists(gds_workflow_path):
        errors.append(f"Missing gds workflow file: {gds_workflow_path}")
    else:
        with open(gds_workflow_path, "r") as f:
            content = f.read()
        if "precheck:" not in content:
...
def check_pdk_drc_rule_compatibility(repo_root="."):
    drc_script_path = os.path.join(repo_root, "tcl/magic_drc.tcl")
    fix_gds_path = os.path.join(repo_root, "py/fix_gds.py")
    errors = []

    if not os.path.exists(drc_script_path):
        errors.append(f"Missing DRC script: {drc_script_path}")
    else:
        with open(drc_script_path, "r") as f:
            content = f.read()
    ...
    if not os.path.exists(fix_gds_path):
        errors.append(f"Missing GDS post-processing script: {fix_gds_path}")
    else:
        with open(fix_gds_path, "r") as f:
            content = f.read()
        if "189" not in content or "235" not in content:
            errors.append("Missing expected prBoundary layer remapping logic in py/fix_gds.py")
...
def main():
    ...
    schema_compat_ok = check_info_yaml_schema_compatibility(repo_root)
