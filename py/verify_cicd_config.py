...
def check_upstream_pdk_action_tags(repo_root="."):
    ...
    for path in [gds_path, docs_path]:
        ...
        if not os.path.exists(path):
            ...
            for tag in actions:
                if tag != "ttihp26b":
                    errors.append(f"Invalid action tag '@{tag}' in {filename}; expected '{expected_tag}'")
...
def check_pdk_drc_rule_compatibility(repo_root="."):
    ...
    if not os.path.exists(drc_script_path):
        ...
        if "drc euclidean on" not in content:
            errors.append("Missing 'drc euclidean on' rule in tcl/magic_drc.tcl")
        if 'drc style "drc(full)"' not in content and "drc style drc(full)" not in content:
            errors.append("Missing 'drc style \"drc(full)\"' rule in tcl/magic_drc.tcl")
    ...
    if not os.path.exists(info_path):
        ...
        for section in required_sections:
            if section not in content:
                errors.append(f"Missing required section '{section}' in info.yaml")

        required_keys = ["top_module:", "tiles:", "uses_vapwr:", "analog_pins:"]
