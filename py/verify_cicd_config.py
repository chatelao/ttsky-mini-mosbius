...
def check_upstream_pdk_action_tags(repo_root="."):
    ...
    for path in [gds_path, docs_path]:
        ...
        if not os.path.exists(path):
            ...
            with open(path, "r") as f:
                content = f.read()
            action_refs = re.findall(r"TinyTapeout/tt-gds-action/[a-zA-Z0-9_-]+@([a-zA-Z0-9_\.-]+)", content)
            if not action_refs:
                errors.append(f"No tt-gds-action references found in {filename}")
            for tag in action_refs:
                if tag != "ttihp26b":
                    errors.append(f"Found non-matching or unpinned tt-gds-action tag '@{tag}' in {filename} (expected '@ttihp26b')")
...
def check_precheck_report_artifacts(repo_root="."):
    ...
    if not os.path.exists(gds_path):
        ...
        if not precheck_match:
            errors.append("Could not find precheck job definition in gds.yaml")
        else:
            precheck_block = precheck_match.group(0)
            if "TinyTapeout/tt-gds-action/precheck@ttihp26b" not in precheck_block:
                errors.append("Missing precheck@ttihp26b action step in precheck job in gds.yaml")
...
def check_pdk_drc_rule_compatibility(repo_root="."):
    ...
    if not os.path.exists(drc_script_path):
        errors.append(f"Missing DRC TCL script: {drc_script_path}")
    else:
        with open(drc_script_path, "r") as f:
            drc_content = f.read()
        if "drc euclidean on" not in drc_content:
