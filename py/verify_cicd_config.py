    ...
    for path in [gds_path, docs_path]:
        ...
        if not os.path.exists(path):
            ...
            for tag in tt_actions:
                if f"@{tag}" != approved_tag:
                    errors.append(f"Unapproved TinyTapeout action tag '@{tag}' in {filename} (expected '{approved_tag}')")
    ...
    for path in [gds_path, docs_path]:
        ...
        if not os.path.exists(path):
            ...
            if not re.search(r"^on:\s+", content, re.MULTILINE):
                errors.append(f"Missing top-level 'on' key in {filename}")
...
def check_info_yaml_schema_compatibility(repo_root="."):
    ...
    if not os.path.exists(info_path):
        ...
        required_keys = ["project:", "title:", "author:", "description:", "top_module:", "language:"]
