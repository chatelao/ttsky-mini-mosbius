...
def check_upstream_pdk_action_tags(repo_root="."):
    ...
    for path in [gds_path, docs_path]:
        ...
        if not os.path.exists(path):
            ...
            for tag in matches:
                if tag != "ttihp26b":
                    errors.append(f"Invalid PDK action tag '@{tag}' in {filename}; expected '@ttihp26b'")
...
def check_workflow_schema_integrity(repo_root="."):
    ...
    if not os.path.exists(gds_path):
        ...
        with open(gds_path, "r") as f:
            content = f.read()
        for job in ["gds:", "precheck:", "viewer:"]:
            if job not in content:
                errors.append(f"Missing job '{job[:-1]}' in gds.yaml schema")

    if not os.path.exists(docs_path):
        errors.append(f"Missing workflow file: {docs_path}")
    else:
        with open(docs_path, "r") as f:
            content = f.read()
        if "docs:" not in content:
            errors.append("Missing job 'docs' in docs.yaml schema")
...
def check_precheck_report_artifacts(repo_root="."):
    ...
    if not os.path.exists(gds_path):
        ...
        if "precheck:" not in content:
            errors.append("Missing precheck job definition in gds.yaml")
        if "continue-on-error: true" not in content:
            errors.append("Missing 'continue-on-error: true' flag in precheck job for report archiving")
...
def check_info_yaml_schema_compatibility(repo_root="."):
    ...
    if not os.path.exists(info_path):
        ...
        with open(info_path, "r") as f:
            content = f.read()
        required_fields = ["title:", "author:", "description:", "pinout:", "top_module:", "tiles:", "analog_pins:"]
...
def main():
    ...
    schema_integrity_ok = check_workflow_schema_integrity(repo_root)
