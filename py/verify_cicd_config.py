...
def check_upstream_pdk_action_tags(repo_root="."):
    ...
    for path in [gds_path, docs_path]:
        ...
        if not os.path.exists(path):
            ...
            with open(path, "r") as f:
                content = f.read()
            tags = re.findall(r"TinyTapeout/tt-gds-action/[a-z_]+@([a-zA-Z0-9_-]+)", content)
            for tag in tags:
                if tag != "ttihp26b":
                    errors.append(f"Invalid upstream action tag '{tag}' in {filename}, expected '@ttihp26b'")
            if "@ttsky" in content:
...
def check_info_yaml_schema_compatibility(repo_root="."):
    ...
    for sec in required_sections:
        if sec not in content:
            errors.append(f"Missing required top-level section '{sec}' in info.yaml")

    required_project_keys = ["top_module:", "tiles:", "analog_pins:", "uses_vapwr:", "language:"]
...
def check_pdk_drc_rule_compatibility(repo_root="."):
    makefile_path = os.path.join(repo_root, "mag/Makefile")
    ...
    if not os.path.exists(makefile_path):
        ...
        with open(makefile_path, "r") as f:
            content = f.read()
        if "ihp-sg13g2.magicrc" not in content:
            errors.append("mag/Makefile missing 'ihp-sg13g2.magicrc' configuration")

    if not os.path.exists(drc_tcl_path):
        errors.append(f"Missing file: {drc_tcl_path}")
    else:
        with open(drc_tcl_path, "r") as f:
            content = f.read()
        if "drc style" not in content or "drc check" not in content:
            errors.append("tcl/magic_drc.tcl missing DRC style or check commands")
...
def check_precheck_report_artifacts(repo_root="."):
    ...
    if not os.path.exists(gds_path):
        ...
        if "TinyTapeout/tt-gds-action/precheck@ttihp26b" not in content:
            errors.append("Missing precheck action step in gds.yaml")
    ...
    for path in [gds_path, docs_path]:
        ...
        if not os.path.exists(path):
            ...
            if "on:" not in content:
                errors.append(f"Missing 'on:' trigger block in {filename}")
...
def main():
    ...
    pdk_drc_rule_ok = check_pdk_drc_rule_compatibility(repo_root)
