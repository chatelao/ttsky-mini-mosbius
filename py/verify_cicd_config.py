    ...
    for path in [gds_path, docs_path]:
        ...
        if not os.path.exists(path):
            ...
            for tag in tt_actions:
                if tag != "ttihp26b":
                    errors.append(f"Action tag mismatch in {filename}: expected 'ttihp26b', got '{tag}'")
    ...
    for path in [gds_path, docs_path]:
        ...
        if not os.path.exists(path):
            ...
            if not re.search(r"^name:\s*\S+", content, re.MULTILINE):
                errors.append(f"Missing workflow name declaration in {filename}")
...
def check_precheck_report_artifacts(repo_root="."):
    ...
    if not os.path.exists(gds_path):
        ...
        if "precheck:" not in content:
            errors.append("Missing precheck job definition in gds.yaml")
        if "precheck@ttihp26b" not in content:
            errors.append("Missing precheck@ttihp26b action reference in gds.yaml")
        if "continue-on-error: true" not in content:
            errors.append("Missing continue-on-error setting for precheck reporting in gds.yaml")
...
def check_pdk_drc_rule_compatibility(repo_root="."):
    ...
    if not os.path.exists(drc_path):
        errors.append(f"Missing TCL script: {drc_path}")
    else:
        with open(drc_path, "r") as f:
            drc_content = f.read()
        if "drc euclidean on" not in drc_content:
            errors.append("Missing 'drc euclidean on' setting in tcl/magic_drc.tcl")
        if "drc style" not in drc_content:
            errors.append("Missing 'drc style' specification in tcl/magic_drc.tcl")
    ...
    if not os.path.exists(gds_path):
        ...
        if "pdk: ihp-sg13g2" not in gds_content:
            errors.append("Missing 'pdk: ihp-sg13g2' configuration in gds.yaml")
...
def check_info_yaml_schema_compatibility(repo_root="."):
    ...
    if not os.path.exists(info_path):
        ...
        required_keys = ["project:", "top_module:", "language:", "tiles:", "pinout:"]
